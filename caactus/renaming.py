...
The renaming step is especially relevant when processing large datasets. 

Create a CSV file that contains metadata in columns.
Each row corresponds to one image. 

The order of rows should match the order of the input files. 

The new file name will be generated by concatenating the column names and their corresponding values with "-" for each row.

The columns will be separated by underscores. 
...
CAVE: The only hardcoded column names needed are "timepoint", "biorep", and "techrep".
"""
